...
<!-- ![Pix2Pix Pipeline](assets/pix2pix_pipeline.png) -->
...
<!-- 🖼️ IMAGE 7: Use the "Epochwise Results" grid from Slide 13 of 
     PROJECT_PRESENTATION.pdf, showing outputs at epochs 01–04 across four 
     different characters. This is one of the strongest visuals in the deck — 
     definitely include it. Save as assets/epochwise_results.png -->
<!-- ![Epoch-wise Results](assets/epochwise_results.png) -->

The model's output quality improves visibly across training epochs — early outputs are blurry/unstructured, while later epochs produce cleaner, more Hindi-character-like shapes.

<!-- 🖼️ IMAGE 8 (optional): If you still have them, add the FID score bar chart 
     and the discriminator/generator loss plots generated at the end of the 
     notebook (final_loss_plot.png, final_Individual_g_loss). These aren't in 
     the presentation but the notebook code generates them — worth re-running 
     and saving to assets/ if you don't already have the files. -->
<!-- ![Loss Curves](assets/final_loss_plot.png) -->

Model performance was also tracked using **FID (Fréchet Inception Distance)** scores across training steps, alongside generator/discriminator loss curves.

## ⚠️ Limitations

- Small dataset, especially for Sharda (only 66 pairs) — likely limits generalization for that script.
- GAN training is inherently unstable; results can vary across runs.
- No formal quantitative accuracy/readability metric beyond FID and visual inspection.
- Currently limited to single isolated characters, not full words or inscriptions.
...
- [Project Presentation](#) — see linked slides for full visual walkthrough.
